...
func (loginRes *LoginResPacket) DecodingPacket(data []byte) {
	reader := MakeReader(data, true)
...
type RoomLeaveReqPacket struct{}

func (request RoomLeaveReqPacket) EncodingPacket() ([]byte, int16) {
	totalSize := _clientSessionHeaderSize
	sendBuf := make([]byte, totalSize)

	writer := MakeWriter(sendBuf, true)
	EncodingPacketHeader(&writer, totalSize, PACKET_ID_ROOM_LEAVE_REQ, 0)
	return sendBuf, totalSize
}
...
func (notify *RoomNewUserNtfPacket) Decoding(bodyData []byte) {
	reader := MakeReader(bodyData, true)
	notify.User = reader.ReadBytes(len(bodyData))
}
